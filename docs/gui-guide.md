...
![图形界面主页：五页标签、选本体与能力标签、模拟模式开关、任务卡片与运行/配置按钮](images/gui-home.png)

顶部标签在 **主页 / 配置 / 运行 / 历史 / 设置** 五页间切换。上图的**主页**是主入口：左侧下拉选
**本体**（如 *Piper 六轴机械臂*），右上角「🧪 模拟
模式」开关决定是否连真实硬件与大模型运行，中间点选**任务**卡片，下方用「▶ 运行 / ⚙ 配置」操作
当前任务，点击运行按钮一键运行任务。
