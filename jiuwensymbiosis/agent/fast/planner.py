...
_COMPILER_SYSTEM = (
    ...
    "- **把 SKILL.md workflow 表里写成裸名的检测字段改写成 <bind>.字段**：SKILL.md 为可读常把检测结果写成"
    "裸字段名；展开成动作序列时必须改成 <bind>.字段 的形式，并给产生这些值的那一步补上对应的 bind。"
    "**漏了 bind，后面的 <bind>.字段 就引用了未绑定的名字，整条序列会被判非法而作废。**\n"
    ...
    "用途不同；后续每一处 <bind>.字段 里的 <bind> 都必须和产生它的那个检测步的 bind 一模一样，"
    "否则运行时读不到该检测结果。\n"
    "- params 的值可以是数字，或对【已绑定检测】的算术表达式（只允许 + - * / 和 字段/下标），"
    '例如 "obj.z"、"obj.position[0]"（obj 指代你取的某个 bind 名）。目标名等字符串原样写。\n'
    "- **严格照 SKILL.md 展开：workflow 表逐行、正文里的注意事项与步骤顺序约束也一并遵守，不要自己加额外步骤或偏移**。"
    ...
    *,
    ...
    for attempt in range(1, max(1, attempts) + 1):
        ...
        try:
            ...
            correction = (
                f"\n\n上次输出的动作序列无效：{exc}\n"
                "请修正后重新输出完整的 JSON 数组。特别注意：凡是产生坐标供后续步骤读取的检测步都必须带 bind；"
                "后续步骤引用检测字段（<bind>.字段）时，其中的 <bind> 必须与产生它的那个检测步的 bind 名"
                "逐字一致（用下划线，不含空格；object_name 是另一个独立的自由字符串）。"
            )
            continue
