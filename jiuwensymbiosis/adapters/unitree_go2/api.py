...
"""``UnitreeGo2Api`` — Unitree Go2 mobile-base capability API.
...
and ``z`` is ignored (a quadruped base doesn't actuate world-frame Z).
...
class UnitreeGo2Api(
    ...
    """Unitree Go2 mobile-base API — chassis motion + camera stream."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize UnitreeGo2Api with env and (optional) detection geometry constants."""
        super().__init__(env)
    ...
    def goto_xyzr(self, x: float, y: float, z: float = 0.0, r: float | None = None) -> None:
        ...
        (a quadruped base doesn't move in world-frame Z). ``rx, ry`` are fixed
    ...
    def get_grasp_info_simple(self, object_name: str) -> dict:
        ...
        raise NotImplementedError("UnitreeGo2Api.get_grasp_info_simple: no arm on this form factor")
    ...
    def pixel_to_base_xyz(self, u: float, v: float, depth_m: float) -> dict:
        ...
        raise NotImplementedError("UnitreeGo2Api.pixel_to_base_xyz: no calibration on this form factor")

    def analyze_scene(self, object_name: str | None = None) -> dict:
        """Not implemented — override to wire up a VLM scene-analysis backend."""
        raise NotImplementedError("UnitreeGo2Api.analyze_scene: not implemented")
