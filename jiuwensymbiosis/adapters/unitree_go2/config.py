...
class UnitreeGo2Config:
    ...
    # ``x_min/max`` etc. bound the base's XY roaming range (meters, not mm —
    # base-frame units differ from arm-flange mm; SafetyRail only checks the
    # values, not their unit).
    z_min_safe_mm: float = 0.0
    x_min_mm: float | None = -5.0
    x_max_mm: float | None = 5.0
    y_min_mm: float | None = -5.0
    y_max_mm: float | None = 5.0
