...
class CruzrConfig:
    ...
    left_raise_position_rad: float | None = None
    right_raise_position_rad: float | None = -1.0
    ...
    camera_calib_path: str | None = None
    ...
    home_clearance_arm_q: dict = field(
    ...
    grasp_face_flatness_max: float = (
        0.15  # 面法向可信阈值 λmin/λmid：小于此才认为看到平整正面、据其绕正；否则退回径向靠近
    )
    ...
    place_edge_quality_max: float = (
        0.25  # 近边拟合可信阈值 edge_quality=残差厚度/边长;≤此才用近边法向对正,否则退回 footprint。越小越严
    )
