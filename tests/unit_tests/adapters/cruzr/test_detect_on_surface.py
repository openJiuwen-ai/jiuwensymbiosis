...
def _api():
    ...
    api._seg_fn = lambda rgb, text_prompt=None: [
        {"mask": np.ones((4, 4), bool), "score": 0.8},
...
def _geo(center):
    return og.ObjectGeometry3D(
        ok=True,
        reason="",
        center_mm=center,
        width_mm=200,
        height_mm=100,
        front_x_mm=center[0] - 50,
        top_z_mm=center[2] + 50,
        n_points=500,
...
def test_picks_the_target_on_the_reference(monkeypatch):
    ...
    monkeypatch.setattr(scene3d, "_calibrated_frame_or_reason", lambda *a: (_EMPTY_FRAME, None))
    r = scene3d.locate_for_grasp(api, "white box", "brown table", "on")
    assert r["ok"] is True
...
def test_reference_not_found_fails(monkeypatch):
    api = _api()
    monkeypatch.setattr(
        scene3d, "_sense_surface_plain", lambda _api, name, *a, **k: {"ok": False, "reason": "surface_not_found"}
    )
    r = scene3d._detect_related(api, "white box", "brown table", "on", _EMPTY_FRAME)
...
def test_picks_the_surface_holding_the_reference(monkeypatch):
    ...
    monkeypatch.setattr(scene3d, "_calibrated_frame_or_reason", lambda *a: (_EMPTY_FRAME, None))
    r = scene3d.locate_for_place(api, "table", "water cup", "under")
    assert r["ok"] is True
...
def test_beside_picks_the_neighbour_not_the_stack(monkeypatch):
    ...
    monkeypatch.setattr(
        scene3d,
        "_candidate_geometries",
        lambda _api, name, *a, **k: (
            [(hat, {})] if name == "hat" else [(_geo((650.0, 350.0, 650.0)), {}), (_geo((650.0, -900.0, 650.0)), {})]
        ),
    )
...
def test_the_place_side_takes_the_relation_too(monkeypatch):
    ...
    tables = [_surf((650.0, 0.0, 600.0), 400.0, 900.0, 600.0), _surf((650.0, 900.0, 600.0), 400.0, 900.0, 600.0)]
    monkeypatch.setattr(
        scene3d,
        "_candidate_geometries",
        lambda _api, name, *a, **k: [(cup, {})] if name == "water cup" else [(t, {}) for t in tables],
    )
    r = scene3d._sense_surface_related(api, "table", "water cup", "under", _EMPTY_FRAME)
