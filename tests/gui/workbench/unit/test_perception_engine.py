# coding: utf-8
# Copyright (c) Huawei Technologies Co., Ltd. 2026. All rights reserved.

"""perception_engine:后台线程 + 事件队列驱动相机预览与点选反投影(纯逻辑,无 nicegui)。

用 scene-backed 会话(``MockArmEnv(scene=MockScene)`` + ``SceneMockApi``,跑真实的
pixel→base 反投影)经 ``session_factory`` 注入,验证点选取点自洽;并用无深度的模拟会话
验证「无深度相机」的错误引导路径。
"""

from __future__ import annotations

import time
from types import SimpleNamespace
from typing import Any

import pytest

from jiuwensymbiosis.agent.lifecycle import CleanupReport, HardwareCleanupError
from jiuwensymbiosis.agent.session import RobotSession
from jiuwensymbiosis.env.mock import MockArmEnv
from jiuwensymbiosis_gui.workbench.perception_engine import PerceptionEngine
from tests.mocks.mock_scene import MockObject, MockScene
from tests.mocks.mock_session import make_mock_session
from tests.mocks.scene_api import SceneMockApi


class _Binding:
    def __init__(self, session) -> None:
        self.session = session

    def build_session(self):
        return self.session


class _Admission:
    def __init__(self, *, binding=None, error: Exception | None = None) -> None:
        self.binding = binding
        self.error = error
        self.calls: list[tuple[dict[str, Any], str, Any]] = []
        self.reports: list[CleanupReport] = []

    def acquire(self, config_snapshot, operation, *, stop=None):
        self.calls.append((config_snapshot, operation, stop))
        if self.error is not None:
            raise self.error
        return self.binding, object()

    def finish(self, _lease, report: CleanupReport) -> bool:
        self.reports.append(report)
        return report.released


class _PreviewSession:
    def __init__(self, report: CleanupReport | None = None) -> None:
        self.env = SimpleNamespace(get_observation=lambda: SimpleNamespace(rgb=None, depth=None))
        self.api = SimpleNamespace()
        self.name = "preview-test"
        self.report = CleanupReport() if report is None else report
        self.disconnect_calls = 0

    def connect(self) -> None:
        pass

    def disconnect(self) -> None:
        self.disconnect_calls += 1

    def cleanup_report(self) -> CleanupReport:
        return self.report


def _scene_session() -> tuple[RobotSession, MockArmEnv]:
    """一个 scene-backed 会话:相机看向桌面中心一个高 50mm 的盒子(顶面投影到画面中心)。"""
    scene = MockScene(
        objects=[MockObject(name="box", base_xy_mm=(0.0, 0.0), size_mm=(80.0, 80.0, 50.0), color=(200, 50, 50))]
    )
    env = MockArmEnv(scene=scene)
    api = SceneMockApi(env)
    return RobotSession(env=env, api=api, name="scene"), env


def _drain_until(engine: PerceptionEngine, want_tag: str, timeout: float = 5.0) -> list[tuple[str, object]]:
    """轮询 ``drain()`` 累积事件,直到出现 ``want_tag`` 或超时。"""
    deadline = time.monotonic() + timeout
    events: list[tuple[str, object]] = []
    while time.monotonic() < deadline:
        events.extend(engine.drain())
        if any(tag == want_tag for tag, _ in events):
            return events
        time.sleep(0.02)
    return events


@pytest.mark.unit
def test_locates_clicked_pixel_and_applies_z_correction():
    session, env = _scene_session()
    engine = PerceptionEngine(lambda: session, z_correction_mm=-57.0)
    engine.start()
    try:
        started = _drain_until(engine, "frame")
        assert any(tag == "preview_started" for tag, _ in started)
        assert any(tag == "frame" for tag, _ in started)

        # 画面中心 (320,240) 正对盒子顶面中心 → 反投影应回到基座 (0,0,50)。
        engine.request_point(320, 240)
        events = _drain_until(engine, "point_result")
        results = [p for tag, p in events if tag == "point_result"]
        assert results, "expected a point_result event"
        r = results[-1]
        assert r["ok"] is True
        assert abs(r["x"] - 0.0) < 2.0
        assert abs(r["y"] - 0.0) < 2.0
        assert abs(r["z"] - 50.0) < 2.0
        # z 校正为展示层叠加,不改反投影本身。
        assert abs(r["z_corrected"] - (50.0 - 57.0)) < 2.0
        assert r["z_correction_mm"] == -57.0
    finally:
        engine.stop()
        engine.join(timeout=5.0)

    assert not engine.is_running()
    assert env._connected is False  # 停止后应断开相机


@pytest.mark.unit
def test_out_of_range_click_reports_reason_without_crashing():
    session, _env = _scene_session()
    engine = PerceptionEngine(lambda: session)
    engine.start()
    try:
        _drain_until(engine, "frame")
        engine.request_point(9999, 9999)  # 越界
        events = _drain_until(engine, "point_result")
        results = [p for tag, p in events if tag == "point_result"]
        assert results and results[-1]["ok"] is False
        assert "范围" in str(results[-1]["reason"])
    finally:
        engine.stop()
        engine.join(timeout=5.0)


@pytest.mark.unit
def test_reports_error_when_no_depth():
    # 无 scene 的模拟会话:get_observation 有 rgb 但 depth 为 None。
    session = make_mock_session()
    engine = PerceptionEngine(lambda: session)
    engine.start()
    try:
        events = _drain_until(engine, "error")
        errors = [p for tag, p in events if tag == "error"]
        assert errors, "expected an error event when depth is unavailable"
        assert "深度" in str(errors[0]["reason"])
    finally:
        engine.stop()
        engine.join(timeout=5.0)

    assert not engine.is_running()


def test_admission_failure_does_not_build_or_connect_a_session():
    admission = _Admission(error=RuntimeError("task already owns the camera"))
    factory_calls: list[str] = []
    engine = PerceptionEngine(lambda: factory_calls.append("built"), admission=admission, config_snapshot={"x": 1})

    engine.start()

    events = engine.drain()
    assert not engine.is_running()
    assert not factory_calls
    assert [tag for tag, _ in events] == ["error", "preview_stopped"]
    assert "task already owns" in events[0][1]["reason"]


def test_admitted_preview_uses_binding_and_keeps_cleanup_failure_blocked():
    session = _PreviewSession(CleanupReport(errors=("camera disconnect failed",)))
    binding = _Binding(session)
    admission = _Admission(binding=binding)
    fallback_calls: list[str] = []
    engine = PerceptionEngine(
        lambda: fallback_calls.append("used"),
        admission=admission,
        config_snapshot={"adapter": "fake"},
    )
    engine.start()
    engine.join(timeout=2.0)

    events = engine.drain()
    assert not fallback_calls
    assert admission.calls[0][1] == "perception_preview"
    assert admission.reports == [session.report]
    assert session.disconnect_calls == 1
    assert any(tag == "error" and "camera disconnect failed" in payload["reason"] for tag, payload in events)


def test_thread_start_failure_releases_prepared_maintenance_lease(monkeypatch):
    from jiuwensymbiosis_gui.workbench import perception_engine as perception_module

    class FailedThread:
        def __init__(self, **_kwargs) -> None:
            pass

        def start(self) -> None:
            raise RuntimeError("thread start failed")

        def is_alive(self) -> bool:
            return False

    session = _PreviewSession()
    admission = _Admission(binding=_Binding(session))
    monkeypatch.setattr(perception_module, "Thread", FailedThread)
    engine = PerceptionEngine(lambda: session, admission=admission, config_snapshot={"adapter": "fake"})

    engine.start()
    engine.stop()
    engine.join(timeout=1.0)

    assert not engine.is_running()
    assert admission.reports == [CleanupReport()]
    events = engine.drain()
    assert any(tag == "error" and "thread start failed" in payload["reason"] for tag, payload in events)


def test_binding_cleanup_error_without_a_session_keeps_the_lease_blocked():
    class BrokenBinding:
        def build_session(self):
            raise HardwareCleanupError("build perception session", cleanup_errors=(RuntimeError("sidecar stop"),))

    admission = _Admission(binding=BrokenBinding())
    engine = PerceptionEngine(lambda: None, admission=admission, config_snapshot={"adapter": "fake"})

    engine.start()
    engine.join(timeout=2.0)

    assert admission.reports
    assert not admission.reports[0].released
    assert "hardware cleanup unconfirmed" in admission.reports[0].errors[-1]
    assert "sidecar stop" in admission.reports[0].errors[-1]
    assert engine._lease is not None
