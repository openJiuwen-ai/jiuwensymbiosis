...
# SO-101 真机环境（安装 LeRobot；0.6.x 版本需要 Python 3.12；建议同时安装 full，避免重新安装 torch）
...
adapters/    本体适配层 (piper/ + so101/ + _common/ 通用构建器)
...
* **本体适配层**：`piper/` 与 `so101/` 示范了如何接入具体本体；`_common/builder.py` 提供通用多态会话工厂。
